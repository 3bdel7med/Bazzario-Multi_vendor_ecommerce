...
<div class="container py-5" dir="ltr">
    ...
    @else
        <div class="row g-4">
            ...
            <!-- Right Side: Order Summary Card & Checkout Form -->
            <div class="col-12 col-lg-4">
                <div class="card p-4 border border-light shadow-sm rounded-3 h-100 d-flex flex-column justify-content-between">
                    ...
                    <form action="{{ route('customer.checkout.store') }}" method="POST" class="mt-2 pt-3 border-top border-light">
                        @csrf
                        <div class="mb-3">
                            <label class="form-label text-muted small font-weight-semibold mb-1">Full Shipping Address</label>
                            <textarea name="shipping_address" rows="3" class="form-control text-sm shadow-inner" placeholder="Enter your full street address, city, country, and phone number..." required></textarea>
                        </div>
                        <button type="submit" class="btn btn-primary w-full py-3 font-weight-bold shadow-sm text-center text-sm">
